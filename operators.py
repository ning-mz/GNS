...
fixed_order_ops = [
    ...
    'RNgon_B_Area', 'RNgon_L_Area', 'RNgon_H_Area', 'Cone_Area', 'Only_sin', 'Only_cos', 'Only_tan']
alterable_order_ops = [
...
def operand_update(operands, ANNO): # 更新V0为实际值 TODO
    for id in range(len(operands)):
        # Substitute variable
        if operands[id][0] != 'V':
            operands[id] = "("+operands[id]+")"
            # pi
...
def Cone_Area(arg_list):
    """
        Cone_Area(a, b, c) -> pi*a*b=c
    """
    if len(arg_list) != 3:
        print("<Cone_Area> function has 2 augments!")
        raise Exception
    expr_step = '3.141593*'+arg_list[0]+'*'+arg_list[1]+'-'+arg_list[2]
    return expr_step
...
def Only_sin(arg_list):
    """
        Only_sin(a, b) -> sin(a)=b
    """
    if len(arg_list)!=2:
        print("<Only_sin> function has 2 augments!")
        raise Exception
    expr_step = '\\sin{'+arg_list[0]+'/180*3.141593}'+'-'+arg_list[1]
    return expr_step

def Only_cos(arg_list):
    """
        Only_cos(a, b) -> cos(a)=b
    """
    if len(arg_list)!=2:
        print("<Only_cos> function has 2 augments!")
        raise Exception
    expr_step = '\\cos{'+arg_list[0]+'/180*3.141593}'+'-'+arg_list[1]
    return expr_step

def Only_tan(arg_list):
    """
        Only_tan(a, b) -> tan(a)=b
    """
    if len(arg_list)!=2:
        print("<Only_tan> function has 2 augments!")
        raise Exception
    expr_step = '\\tan{'+arg_list[0]+'/180*3.141593}'+'-'+arg_list[1]
    return expr_step
...
def mid_var_update_before_step(current_step, ANNO): # Do before each step start
    for mid_var in ANNO.mid_varible_dict:
        if mid_var[0] == 'V': # jump over V0
            continue

        for ind, token in enumerate(current_step):
            if token not in arith_op_list:
                if mid_var in token:
                    new_token = token.replace(mid_var, "("+ANNO.mid_varible_dict[mid_var]+")")
                    current_step[ind] = new_token
    return current_step

def result_compute(exp_tokens):
    ANNO = Varible_Record()
    # Obtain the mapping between lowercase letters to intermediate variables V_i
    visit_list = [] # arguments denoted by lowercase letters，检查变量中用到的字母，需要保留
    for token in exp_tokens:
        if token in arith_op_list:
            continue
        for char in token:
            if char in low_case_list: visit_list.append(char)
    visit_list.sort() 
    ...
    # mapping between letters to intermediate variables V_i，替换V0临时变量为 b c 等字符
    midvar2letter = dict() 
    midletter2var = dict() 
